...
- **It builds, its checks pass, it is formatted and lint-clean.**
...
- **Refactors precede the change that needs them.** A `refactor:`
  ...
  the move, rename or extraction did not alter what the image contains;
  the behavior change is in the `feat:` or `fix:` that follows.
...
- **Every commit must build and pass all checks.** No "WIP" commits, no
  commits that leave the tree broken and rely on a follow-up to fix it.
- **Every commit must be formatted and lint-clean** with the tools the
  repository declares. Run them before committing, not after review.
- **Separate preparatory refactorings from behavior changes.** If a fix
  ...
  apparent while writing the behavior change, such as splitting a build
  stage to avoid duplication. Before committing, review the diff and
...
A body worth writing:

```
build(image): download the native binary instead of the npm package

The npm package only wraps the same native binary and would pull node
into the image for nothing. The official install script is not used
either: it always fetches the latest release before installing the
requested one, which defeats the pin.
```
...
- **type** is one of `feat`, `fix`, `refactor`, `test`, `docs`, `build`, `ci`, `chore`.
  `perf` and `style` are not used: a size or build time change is a `fix` or a
  `feat`, and formatting never lands alone since every commit is already formatted.
...
- **breaking changes** are marked with `!` after the type or scope (`build(image)!:`)
  when what a derived image inherits changes incompatibly: a path, an
  environment variable, the user, the entrypoint. Say what breaks and how to
  migrate in the body. Do not use the `BREAKING CHANGE:` footer.
